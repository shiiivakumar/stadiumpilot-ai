// StadiumPilot AI - Client AI Service Bridge
...
// ----------------------------------------------------
export async function askGeminiFan(message, context, lang = 'en') {
  ...
  try {
    const response = await fetch('/api/ai/chat', {
      ...
      body: JSON.stringify({ message: safeMessage, context, lang })
    });

    if (response.ok) {
      const data = await response.json();
      return { text: data.text, isSimulated: data.isSimulated };
    }
  } catch (err) {
    console.warn('Backend API `/api/ai/chat` unreachable. Running local fallback:', err.message);
  }

  // Double safety: client-side fallback if backend failed/unreachable
  return {
    text: generateFallbackFanResponse(safeMessage, context, lang),
    isSimulated: true
  };
}

// ----------------------------------------------------
// 2. OPERATIONS BRIEF ENGINE
// ----------------------------------------------------
export async function generateOperationsBrief(context) {
  try {
    ...
    console.warn('Backend API `/api/ai/brief` unreachable. Running local fallback:', err.message);
  }

  return {
    text: generateFallbackBrief(context),
    isSimulated: true
  };
}

// ----------------------------------------------------
// 3. INCIDENT RESPONSE PLAN GENERATOR
// ----------------------------------------------------
export async function generateIncidentResponsePlan(incident) {
  try {
    const response = await fetch('/api/ai/incident-plan', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    console.warn('Backend API `/api/ai/incident-plan` unreachable. Running local fallback:', err.message);
  }

  return {
    text: generateFallbackIncidentPlan(incident),
    isSimulated: true
  };
}

// ----------------------------------------------------
// CLIENT-SIDE FALLBACK CODE (Offline Safety Net)
// ----------------------------------------------------
